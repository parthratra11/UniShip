...
const products = [
  {
    ...
    customsInfo: {
      ...
      category: ["Apparel", "Clothing", "Textile", "Garment", "Cotton", "T-Shirt", "Men", "Women", "Kids", "Unisex"],
    },
    manufacturingInfo: {
      date: "2024-01-01",
...
const CreateOrderForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const customsBody = {
        description: selectedProduct?.customsInfo?.description || '',
        material_composition: selectedProduct?.customsInfo?.materialComposition || '',
        category: selectedProduct?.customsInfo?.category || [],
        hsn_code: selectedProduct?.hsnCode || '',
        manufacturing_country: selectedProduct?.manufacturingInfo?.manufacturingCountry || '',
        quantity: formData.quantity,
        unit_price: selectedProduct?.mrp || 0,
        currency: selectedProduct?.mrpUnit || 'INR'
      };

      // First create customs declaration
      const customsResponse = await fetch('/api/customs-declaration', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(customsBody)
      });

      if (!customsResponse.ok) {
        throw new Error(`HTTP error! status: ${customsResponse.status}`);
      }

      const data = await customsResponse.json();
      const declarationId = data.object_id;
      ...
      // Add proper error handling here, maybe show an error message to the user
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white p-6 rounded-lg border border-[#D5D9D9]">
            <h2 className="text-xl font-medium mb-4 text-[#0F1111]">Product Details</h2>
          ...
          <div className="bg-white p-6 rounded-lg border border-[#D5D9D9]">
            <h2 className="text-xl font-medium mb-4 text-[#0F1111]">Shipping Details</h2>
            
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-medium mb-3 text-[#0F1111]">Shipper Address</h3>
                <div className="space-y-4">
                  <div className="flex gap-4">
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        className="text-[#FF9900] focus:ring-[#FF9900]"
                      />
                      <span className="ml-2 text-[#0F1111]">Custom Address</span>
                    </label>
                  </div>
              ...
              <div>
                <h3 className="text-lg font-medium mb-3 text-[#0F1111]">Receiver Address</h3>
                <AddressForm
                  type="receiver"
                  formData={formData}
                  setFormData={setFormData}
